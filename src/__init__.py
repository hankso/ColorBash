...
def colorize(text = None,
    ...
    effects:list of str, special effects on text and curser.
            Different effects may mix toghter,
            Command to move curser may be repeated,
            All supported command are listed as follow:
            underline
            italic
            bold
            higherlight
    ...
    blackback:False if your bash background color is white (e.g. MacOS shell), default black, like Ubuntu bash or Windows cmd
            
    Examples
    --------
    In [1]: print(colorize(text='~'*17,effects=['underline'])+
                  colorize(text='ColorBash')+
                  colorize(effects=['left']*14+['bold'])+
                  colorize(effects=['right']*3+['clr2end']))
                  
    Out[1]: \33[4m~~~ColorBash~~~
    '''
